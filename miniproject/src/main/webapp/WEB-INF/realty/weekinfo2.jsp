  ...
  <div class="weekdays">
    ...
    <div class="week_estates">
      <ul>
        ...
        <c:forEach var="a" begin="0" end="${length - 1}" varStatus="no">
        <li>
         <a>
          <span id="${'btag'}${no.index}"></span>
          <div id="${'btitle'}${no.index}"></div>
          <aside id="${'baddr'}${no.index}"></aside>
          <span id="${'bpart'}${no.index}"></span>
          <label id="${'bdate'}${no.index}"></label>
          <div><img id="${'bimg'}${no.index}" src=""></div>
         </a>
        </li>
		</c:forEach>
      </ul>
